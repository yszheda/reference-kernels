...
def custom_kernel(data: input_t) -> output_t:
    """
    DeepSeek-R1 MXFP4 MoE fused kernel implementation.

    Uses AITER's 2-stage Composable Kernel (CK) for optimal performance:
    - Stage 1: Gate + Up projection + SwiGLU activation (MXFP4 GEMM)
    - Stage 2: Down projection (MXFP4 GEMM)
    - LDS intermediate caching between stages
    - Per-1x32 MXFP4 quantization with E8M0 scales
    ...
    # Use AITER 2-stage CK fused_moe kernel
    # Automatically selects optimal block_m based on batch size:
    # - bs=16:   block_m=64
    # - bs=128:  block_m=256
    # - bs=512:  block_m=64
    output = fused_moe(
